#!/bin/bash
set -e

nasm -f bin boot.asm -o boot.bin

gcc -m64 -ffreestanding -c test.c -o test.o
nasm -f elf64 kernel_entry.asm -o kernel_entry.o
ld -nostdlib -Ttext=0x1000 -e _start kernel_entry.o test.o -o test.elf
objcopy -O binary test.elf test.bin

dd if=/dev/zero of=disk.img bs=512 count=2880 status=none
dd if=boot.bin of=disk.img conv=notrunc status=none
dd if=test.bin of=disk.img bs=512 seek=1 conv=notrunc status=none

qemu-system-x86_64 -drive format=raw,file=disk.img -no-reboot -d cpu_reset
