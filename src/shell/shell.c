...
shell_command_t shell_commands[] = {
  ...
  {"hexdump", shell_cmd_hexdump, "Hexdump memory: hexdump <addr> <length>"},
};
...
int shell_cmd_hexdump(int argc, char** argv) {
  ...
  unsigned int addr = (unsigned int)strtoul(argv[1], (void*)0, 0);
  unsigned int length = (unsigned int)strtoul(argv[2], (void*)0, 0);

  if (length == 0 || length > 0xFFFFF) { // Add reasonable limit
    print("Error: length must be between 1 and 1MB\n", LIGHT_RED);
    return 1;
  }


  print("Hexdump of memory at ", LIGHT_CYAN);
  print_hex(addr, 8, LIGHT_CYAN);
  print(" for ", LIGHT_CYAN);
  print_hex(length, 8, LIGHT_CYAN);
  print(" bytes:\n", LIGHT_CYAN);

  hexdump((void*)(unsigned long)addr, length, WHITE);
  return 0;
}
