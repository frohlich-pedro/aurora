...
#define PANEL_COLOR (VGA_BLUE | (VGA_BLUE << 4))
#define PANEL_TEXT_COLOR (VGA_YELLOW | (VGA_BLUE << 4))
#define CONTENT_AREA_COLOR (VGA_BLACK | (VGA_GRAY << 4))
#define CONTENT_TEXT_COLOR VGA_BLACK
...
void print_string(const char* string, unsigned char text_color) {
  ...
  while (*string) {
    ...
    if (*string == '\n') {
      ...
      unsigned char color = (text_color & 0x0F) | CONTENT_AREA_COLOR;
